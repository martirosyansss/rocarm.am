...
test("allows host approval and the brand film iframe and preserves the response", async () => {
  const response = applySecurityHeaders(new Response("ok", { status: 201 }));
  expect(response.status).toBe(201);
  expect(await response.text()).toBe("ok");
  expect(response.headers.get("content-security-policy")).toContain(
    "frame-src 'self' https://auth.higgsfield.app https://auth.higgsfield-dev.app https://www.youtube-nocookie.com;",
  );
